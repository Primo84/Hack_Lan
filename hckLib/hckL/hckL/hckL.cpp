...
__declspec(dllexport) int SetRecive(RecivePacketHandler* RecvHandler)
{
	if (opened == 1)
	{
		Event_h = NULL;
	...
	//	memset(&Packet, 0, sizeof(RecvPack));
		closed = 0;
		CreateThread(NULL, 0, &Fun, (PVOID)RecvHandler, THREAD_PRIORITY_NORMAL, &handle);
		Sleep(500);
	}
	return 0;
}
__declspec(dllexport) int SetReciveMPHandler(RecivePacketHandler* RecvHandler, Userminiport * UserMiniP)
{
	...
	//	MPSR.Operacja = 0;
		...
		CreateThread(NULL, 0, &MPFunR, NULL, THREAD_PRIORITY_NORMAL, &RecvMP_H);
		Sleep(5000);
	}
	return 0;
}

__declspec(dllexport) int SetSendMPHandler(RecivePacketHandler* RecvHandler, Userminiport* UserMiniP)
{
	if (MiniportInited == 1 && MiniportSendHandled == 0)
	{
		...
		CreateThread(NULL, 0, &MPFunS, NULL, THREAD_PRIORITY_NORMAL, &SendMP_H);
		Sleep(5000);
	}
	return 0;
}
__declspec(dllexport) int SendPacket(E_Head* Frame)
{
	HANDLE dev = NULL;
	Dev_Lan* D = NULL;
	DWORD ret, code;
	...
	if (opened == 1)
	{
		...
		rozm = 0;
		zn = (char*)Frame;
		for (i = 0; i < sizeof(E_Head); i++)
		{
			if (*zn != 0) rozm++;
			else break;
		}
		*/
		memset(&SPack, 0, sizeof(SendPack));
		SPack.BindingContext = AdapterOpened.BindingContext;
		//SPack.DataSize = rozm;
		memcpy(&SPack.Frame, &Frame, sizeof(E_Head));
		bl = DeviceIoControl(dev, IO_SEND_PACKET, &SPack, sizeof(SendPack), &code, sizeof(DWORD), &ret, NULL);
		CloseHandle(dev);
	}
	else return -1;
	return 0;
								...
								if (result == S_OK)
								{
									...
									if (SUCCEEDED(result))
									{
										...
										if (SUCCEEDED(result))
										{
											...
											if (result == NETCFG_S_REBOOT || NETCFG_E_NEED_REBOOT || result == S_OK)
											{
