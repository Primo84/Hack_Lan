...
NTSTATUS DriverEntry(IN DRIVER_OBJECT* DriverObject, IN PUNICODE_STRING RegistryPath)
{
	...
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "HLFInstaller star..........");
...
{
	DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "HLFInstaller ending..........");
}
...
int CopyFileSystemF(wchar_t *FilePath)
{
	...
	char* bufor;
	wchar_t DestFilePath[]= L"\\DosDevices\\C:\\Windows\\System32\\drivers\\FHacklan.sys";
	...
	wcscpy(&SourceFilePath[wcslen(SourceFilePath)], L"FHacklan.sys");
